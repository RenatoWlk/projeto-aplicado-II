...
import {
  ...
  DonationStatus,
  DonationResponse,
} from "../../calendar/donator-calendar/donator-calendar.service";
...
export class UserAccountComponent implements OnInit {
  ...
  private initForms(): void {
    this.profileForm = this.fb.group({
      ...
      phone: [
        "",
        [Validators.required, Validators.pattern(/^[\d\(\)\-\s]{11,16}$/)],
      ],
    ...
    this.donationService
      ...
      .subscribe({
        next: (donations: any[]) => {
          ...
          const futureAppointment = sortedDonations.find(
            (d) =>
          ...
          const lastCompleted = sortedDonations.find(
      ...
      .subscribe({
        next: (stats: UserStats) => {
          if (stats.achievements) {
            stats.achievements = this.sortAchievementsByRarity(
              stats.achievements,
            );
  ...
  private sortAchievementsByRarity(achievements: any[]): any[] {
    ...
    return achievements.sort(
      (a, b) => order[a.rarity.toLowerCase()] - order[b.rarity.toLowerCase()],
    );
  }
  ...
  getFormattedResultMessage(
    questionnaire: EligibilityQuestionnaireDTO,
  ): string {
    if (questionnaire.eligible) {
      ...
      return (
        questionnaire.resultMessage ||
        "Algumas das suas respostas indicam que você não está apto para doação no momento. Consulte um profissional de saúde para mais informações."
      );
    }
  }
    ...
    if (!questionnaire.eligible) {
      if (questionnaire.symptoms === true) {
        recommendations.push(
          "Aguarde até estar completamente recuperado dos sintomas",
        );
      }

      if (questionnaire.medications === true) {
        recommendations.push(
          "Consulte sobre a compatibilidade dos seus medicamentos com a doação",
        );
      }

      if (questionnaire.procedures === true) {
        recommendations.push(
          "Aguarde o período de recuperação recomendado após procedimentos médicos",
        );
      }

      if (questionnaire.tattooOrPiercing === true) {
        recommendations.push(
          "Aguarde 12 meses após tatuagem, piercing ou acupuntura",
        );
      }

      if (questionnaire.covidVaccine === true) {
        recommendations.push(
          "Aguarde 48 horas após a vacinação contra COVID-19",
        );
      }

      if (questionnaire.yellowFeverVaccine === true) {
        recommendations.push(
          "Aguarde 30 dias após a vacinação contra febre amarela",
        );
      }

      if (questionnaire.drugs === true) {
        recommendations.push(
          "Procure orientação médica especializada sobre uso de substâncias",
        );
      }
      ...
      if (questionnaire.gender === "Feminino") {
        if (questionnaire.pregnant === true) {
          recommendations.push(
            "Gestantes não podem doar sangue por questões de segurança",
          );
        }
      ...
      if (
        questionnaire.gender === "Masculino" &&
        questionnaire.lastDonationMale === true
      ) {
      ...
      if (recommendations.length === 0) {
        ...
        recommendations.push(
          "Refaça o questionário quando as condições mudarem",
        );
      }
    }
  ...
  getFormattedQuestion(field: string): string {
    const questionMap: { [key: string]: string } = {
      ...
      diseases:
        "Possui doenças cardíacas, diabetes, ou outras condições crônicas?",
      ...
      travelRiskArea:
        "Viajou para áreas de risco (malária, febre amarela) recentemente?",
      ...
      .subscribe({
        next: (donations: DonationResponse[]) => {
          ...
          if (completedDonations.length > 0) {
            if (!this.user) return;
            const lastDonation = completedDonations.sort(
              (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
            )[0];
